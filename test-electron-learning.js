...
app.whenReady().then(async () => {
  console.log('🚀 Electron app ready, starting learning system test...\n');
  
  try {
    // Import services after Electron is ready
...
`;
    ...
    console.log('🧪 Testing Learning Guidance System with Electron...\n');
    ...
    // Stop context analysis to prevent error spam
    await contextService.stopContextAnalysis();
    console.log('✅ Context analysis stopped to prevent errors\n');
    ...
    console.log('\n🎯 Learning Overlay should now be visible on your screen!');
    console.log('   - The overlay shows your current learning step');
    console.log('   - It provides contextual guidance based on your screen');
    console.log('   - You can interact with it to see real-time learning help');

    // Keep the app running so you can see the overlay
    console.log('\n⏳ Keeping app running... Press Ctrl+C to exit');
...
process.on('SIGINT', () => {
  ...
  try {
    // Cleanup services
    if (typeof guidanceService !== 'undefined') {
    ...
    if (typeof learningOverlay !== 'undefined') {
